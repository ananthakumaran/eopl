(ns eopl.core.lazy-lang
  (:use eopl.core.define-datatype)
  (:use eopl.core.env)
  (:use eopl.core.lazy-lang-parser)
...
(defn value-of-operand [rand env]
  (cases expression rand
    (var-exp (var) (apply-env env var))
    (else (newref (a-thunk rand env)))))

(defn value-of-thunk [th]
  (cases thunk th
    (a-thunk (exp env)
             (value-of exp env))))
  ...
  (cases expression exp
    ...
    (var-exp (var)
             (let [ref (apply-env env var)
                   val-thunk (de-ref ref)]
               (if (expval? val-thunk)
                 val-thunk
                 (let [val (value-of-thunk val-thunk)]
                   (setref! ref val)
                   val))))
