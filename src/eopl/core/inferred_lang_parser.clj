(ns eopl.core.inferred-lang-parser
...
(defn primitive-exp [rator rands]
  (let [result (cases expression rator
                      (var-exp (var)
                               (apply
                                (ns-resolve 'eopl.core.inferred-lang-parser
...
(defonce variable-type-serial (atom 0))

(defn reset-fresh-var-serial []
  (reset! variable-type-serial 0))

(defn fresh-var-type []
  (var-type (swap! variable-type-serial inc)))

(def parse-optional-type
  (complex [_ space*
            _ (lit \?)]
           (fresh-var-type)))

(def parse-type
  (alt parse-optional-type
