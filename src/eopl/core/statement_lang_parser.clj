(ns eopl.core.statement-lang-parser
...
(defn primitive-exp [rator rands]
  (let [result (cases expression rator
                 (var-exp (var)
                          (apply
                           (ns-resolve 'eopl.core.statement-lang-parser
...
(def parse-print-stmt
  (complex [_ (lit-conc-seq "print")
            _ space+
            exp parse-expression]
           (print-stmt exp)))
...
(def parse-while-stmt
  (complex [_ (lit-conc-seq "while")
            _ space+
            predicate parse-expression
            _ space+
...
(def-parse-delimited parse-stmts (lit \;) parse-statement)

(def parse-multi-stmt
  (complex [_ (lit \{)
            stmts parse-stmts
            _ space*
            _ (lit \})]
